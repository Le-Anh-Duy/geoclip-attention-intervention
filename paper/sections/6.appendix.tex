...
the location embeddings and a cached offline tile set. No GPU is required ---
on CPU alone one request returns both predictions in about eight seconds, and
proposal generation takes about two seconds. From the organisers we need only
table space, mains power and, where available, an external monitor; no network
access is necessary.
