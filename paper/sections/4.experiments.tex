...
over layer 2 is only $0.64$~km (95\% CI $[-16.83,17.57]$). Thus, the experiment
does not support the hypothesis that individually promising layers combine
synergistically.
...
metrics. The multi-layer row is selected on discovery.}
\label{tab:main}
\small
...
layer 2 remains only $0.92$~km (95\% CI $[-23.66,25.66]$). We also repeated
feature extraction in deterministic FP32 and observed zero maximum absolute
difference. Mixed precision can change nearest-neighbour identities when
gallery similarities are nearly tied; it is therefore used only for candidate
screening, while every value reported above comes from the FP32 pass.

The experiment isolates layer-combination effects, not whether WeDetect regions
are more informative than arbitrary regions; that claim requires an
area-matched random-mask control and a fresh external test set.
