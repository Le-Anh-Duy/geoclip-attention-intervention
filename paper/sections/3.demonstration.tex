...
basemap tiles and reverse-geocoded place names from OpenStreetMap services;
these provide cartographic context but are not part of model inference.

\emph{Technical requirements.} We bring the demonstration machine: a single
laptop holding the $1.6$~GB CLIP ViT-L/14 checkpoint and the $410$~MB
WeDetect-Uni proposal graph. No GPU is required --- on CPU alone, one request
returns both predictions in about eight seconds. We need only table space and
mains power; a cached offline tile set replaces the map service where network
access is unavailable. The implementation and
